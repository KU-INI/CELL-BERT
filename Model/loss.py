...
def focal_loss(input, target, alpha, gamma, reduction, eps, ignore_index, label_weight):
    ...
    target_one_hot = target_one_hot * label_weight * 2
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha, gamma = 2.0, reduction = 'mean', eps = 1e-8, ignore_index=30, weight = [0.5, 0.5]):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, input, target):
        return focal_loss(input, target, self.alpha, self.gamma, self.reduction, self.eps, self.ignore_index, self.weight)